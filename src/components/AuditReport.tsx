      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-slate-200 print:border-0 print:shadow-none print:p-0">
        ...
        <div className="flex flex-wrap gap-3 mt-4 md:mt-0 justify-center print:hidden">
          ...
          >
            ...
            {isUnlocked ? 'Download PDF' : 'Unlock PDF Report'}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 relative overflow-hidden print:border-slate-300 print:break-inside-avoid">
            ...
            <div className={`space-y-4 ${isUnlocked ? '' : 'blur-md select-none opacity-40 pointer-events-none'}`}>
              {[
            ...
            {!isUnlocked && (
              <div className="absolute inset-0 bg-white/20 backdrop-blur-[1px] flex flex-col items-center justify-center text-center p-6 z-10">
                  <div className="bg-white p-5 rounded-xl shadow-xl border border-slate-100 max-w-xs print:shadow-none print:border-slate-300">
                    <Lock className="w-10 h-10 text-yellow-500 mx-auto mb-3" />
                    <h4 className="font-bold text-slate-900 mb-2">Unlock Plan</h4>
                    <p className="text-xs text-slate-500 mb-4 leading-relaxed">
                      This strategy is locked. Upgrade to see the step-by-step fix plan.
                    </p>
                    <button 
                      onClick={() => setShowPricing(true)}
                      className="block w-full py-2.5 bg-gradient-to-r from-yellow-400 to-yellow-500 text-slate-900 rounded-lg text-sm font-bold hover:shadow-lg transition-all print:hidden"
                    >
                      Unlock Now
                    </button>
                  </div>
              ...
              <div className="divide-y divide-slate-100 print:divide-slate-200">
                {gbpFactors.map((factor) => (
                  <FactorRow 
                    ...
                    isLocked={!isUnlocked && factor.status !== 'good'} // Lock issues if free
